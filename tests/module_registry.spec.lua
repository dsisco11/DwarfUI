...
describe('DwarfUI module registry', function()
    it('loads registered contracts in dependency order', function()
        ...
        assert.equals('dwarfui/widget_extensions', calls[3])
        assert.equals('dwarfui/widgets/asset_button', calls[4])
        assert.equals('dwarfui/widgets/hover_action_rail', calls[5])
        assert.equals('dwarfui/pointer', calls[6])
        assert.equals('dwarfui/pointer_poller', calls[7])
        assert.equals('dwarfui/tooltip_root_resolver', calls[8])
        assert.equals('dwarfui/tooltip_target', calls[9])
        assert.equals('dwarfui/tooltip_target_detector', calls[10])
        assert.equals('dwarfui/tooltip_map_target', calls[11])
        assert.equals('dwarfui/tooltip_service', calls[12])
        assert.equals('dwarfui/tooltip_render_hook', calls[13])
    ...
    it('clears consumers before their dependencies', function()
        ...
        assert.equals('dwarfui/tooltip_render_hook', names[#names - 12])
        assert.equals('dwarfui/tooltip_service', names[#names - 11])
        assert.equals('dwarfui/tooltip_map_target', names[#names - 10])
        assert.equals('dwarfui/tooltip_target_detector', names[#names - 9])
        assert.equals('dwarfui/tooltip_target', names[#names - 8])
        assert.equals('dwarfui/tooltip_root_resolver', names[#names - 7])
        assert.equals('dwarfui/pointer_poller', names[#names - 6])
        assert.equals('dwarfui/pointer', names[#names - 5])
        assert.equals('dwarfui/widgets/hover_action_rail',
            names[#names - 4])
        assert.equals('dwarfui/widgets/asset_button', names[#names - 3])
        assert.equals('dwarfui/widget_extensions', names[#names - 2])
