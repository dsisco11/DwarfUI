...
Proposed. This document defines the target public contract and project boundary.
It does not implement DwarfUICore or migrate consumer plugins.

## Decision summary

Extract the process-wide tooltip, context-menu, pointer, registration, and
presentation infrastructure from DwarfUI into a new DFHack Lua project named
DwarfUICore.
...
DwarfUICore owns reusable process-wide infrastructure:
...
DwarfUI remains the higher-level component and feature library. It retains:
...
DwarfUI becomes a normal DwarfUICore consumer. Its core service handles use the
reserved namespace `dwarfui`.

External plugins that need only tooltips or context menus depend directly on
DwarfUICore. They do not depend on DwarfUI's internal layout or initialization.
...
## Extraction and adoption sequence

The eventual implementation should be delivered as coordinated project work:

1. Create DwarfUICore with the root contract, manifest, namespace system,
   providers, immutable API objects, and focused contract tests.
2. Move the tooltip and context-menu systems with their owned transitive
   infrastructure into DwarfUICore without changing their observable mediation
   and presentation behavior.
3. Make DwarfUI depend on the packaged DwarfUICore version and consume services
   through namespace `dwarfui`.
4. Remove DwarfUI's duplicate process owners only after DwarfUI is fully using
   DwarfUICore.
5. Publish and verify both package contents and installation-resolution order.

Consumer plugin migration outside DwarfUI is separate work. The extraction must
not edit those plugins.
